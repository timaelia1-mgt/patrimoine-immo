...
import { getBien, getLoyers } from '@/lib/database'
...
export async function GET(
  ...
  try {
    ...
    // Récupérer les loyers de l'année en cours
    const annee = new Date().getFullYear()
    const loyers = await getLoyers(id, annee)
    
    return NextResponse.json({ loyers })
  } catch (error: unknown) {
    logger.error('[API getLoyers] Erreur:', error)
    return NextResponse.json(
